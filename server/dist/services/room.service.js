...
class RoomService {
    ...
    broadcast(roomId, event, payload) {
        ...
        if (io) {
            ...
            logger_1.logger.warn("Socket.IO instance not initialized yet, cannot broadcast", { event, roomId });
        }
    }
    /**
     * Create a new room in PostgreSQL and initialize in Redis.
     */
    async createRoom(ownerId, name, isPrivate = false, password) {
        const roomCode = await (0, roomCode_1.generateRoomCode)();
        // 1. Write to Postgres
        const room = await prisma_1.prisma.$transaction(async (tx) => {
        ...
        await room_repository_1.roomRepository.createRoomState(room.id, {
            ownerId,
            code: roomCode,
            name,
        });
        await redis_1.redisClient.sAdd("active_rooms", room.id);
        // 3. Add owner as active participant in Redis
        const ownerUser = await prisma_1.prisma.user.findUnique({
            where: { id: ownerId },
            select: { profile: { select: { displayName: true } } },
        });
        const displayName = ownerUser?.profile?.displayName || "Owner";
        await room_repository_1.roomRepository.saveParticipant(room.id, {
            userId: ownerId,
     ...
     */
    async joinRoom(userId, roomCode, socketId, username, password) {
        ...
        const isBanned = await prisma_1.prisma.roomBan.findUnique({
            where: {
                roomId_userId: {
                    roomId: room.id,
                    userId,
        ...
        if (isBanned) {
            if (!isBanned.expiresAt || isBanned.expiresAt > new Date()) {
                ...
                await prisma_1.prisma.roomBan.delete({
                    where: { id: isBanned.id },
                });
            }
        }
        // 4. Upsert RoomMember record in DB
        let member = await prisma_1.prisma.roomMember.findUnique({
        ...
        if (member) {
            ...
            if (member.leftAt) {
                member = await prisma_1.prisma.roomMember.update({
                    where: { id: member.id },
                    data: { leftAt: null },
                });
            }
            ...
            const activeCount = await prisma_1.prisma.roomMember.count({
            ...
            member = await prisma_1.prisma.roomMember.create({
                data: {
                    roomId: room.id,
                    userId,
                    role,
        ...
        let participant = await room_repository_1.roomRepository.getParticipant(room.id, userId);
        ...
        await room_repository_1.roomRepository.saveParticipant(room.id, participant);
        await redis_1.redisClient.sAdd("active_rooms", room.id);
        await room_repository_1.roomRepository.setUserPresence(userId, "online");
        // Track Socket Presence mapping
        await room_repository_1.roomRepository.setSocketPresence(socketId, {
            userId,
            roomId: room.id,
            connectedAt: new Date().toISOString(),
        });
        // Update heartbeat
        await room_repository_1.roomRepository.updateHeartbeat(userId);
        // 6. Broadcast Events
     ...
     */
    async leaveRoom(userId, roomId, socketId) {
        const participant = await room_repository_1.roomRepository.getParticipant(roomId, userId);
        if (!participant)
            return;
        ...
        if (socketId) {
            ...
            for (const id of participant.socketIds) {
                await room_repository_1.roomRepository.removeSocketPresence(id);
            }
        ...
        if (participant.socketIds.length === 0) {
            ...
            await room_repository_1.roomRepository.setUserPresence(userId, "offline");
            await room_repository_1.roomRepository.removeHeartbeat(userId);
            // In Postgres, mark they left
            await prisma_1.prisma.roomMember.updateMany({
                where: { roomId, userId },
                data: { leftAt: new Date() },
            });
            // Remove from active participants list in Redis
            await room_repository_1.roomRepository.removeParticipant(roomId, userId);
    ...
    async handleSocketDisconnect(socketId) {
        const presence = await room_repository_1.roomRepository.getSocketPresence(socketId);
        if (!presence)
            return;
     ...
     */
    async handleAutomaticOwnershipTransfer(roomId, currentOwnerId) {
        ...
        if (eligible.length === 0) {
            logger_1.logger.info(`No other active participants in room ${roomId}. Room remains ownerless until next join.`);
            return;
        }
     ...
     */
    async performOwnershipTransfer(roomId, fromId, toId, reason) {
        ...
        await room_repository_1.roomRepository.updateRoomState(roomId, { ownerId: toId });
        // 5. Update participant roles in Redis
        const prevParticipant = await room_repository_1.roomRepository.getParticipant(roomId, fromId);
        if (prevParticipant) {
            prevParticipant.role = "PARTICIPANT";
            await room_repository_1.roomRepository.saveParticipant(roomId, prevParticipant);
        }
        const nextParticipant = await room_repository_1.roomRepository.getParticipant(roomId, toId);
        if (nextParticipant) {
            nextParticipant.role = "OWNER";
            await room_repository_1.roomRepository.saveParticipant(roomId, nextParticipant);
        }
        // 6. Broadcast Event
     ...
     */
    async transferOwnershipManually(roomId, currentOwnerId, newOwnerId) {
        const isAllowed = await permission_service_1.permissionService.hasPermission(currentOwnerId, roomId, "TRANSFER_OWNER");
        if (!isAllowed) {
        ...
        const newOwnerMember = await prisma_1.prisma.roomMember.findUnique({
     ...
     */
    async kickUser(moderatorId, roomId, targetUserId, reason) {
        const isAllowed = await permission_service_1.permissionService.hasPermission(moderatorId, roomId, "KICK_USER");
        if (!isAllowed) {
            throw new Error("Unauthorized to kick user");
        }
        const targetParticipant = await room_repository_1.roomRepository.getParticipant(roomId, targetUserId);
        ...
        await prisma_1.prisma.moderationAction.create({
            data: {
                roomId,
                moderatorId,
                targetUserId,
        ...
        if (targetParticipant) {
            ...
            await room_repository_1.roomRepository.removeParticipant(roomId, targetUserId);
        }
     ...
     */
    async banUser(moderatorId, roomId, targetUserId, reason, expiresAt) {
        const isAllowed = await permission_service_1.permissionService.hasPermission(moderatorId, roomId, "BAN_USER");
        if (!isAllowed) {
            throw new Error("Unauthorized to ban user");
        }
        // 1. Record ban in Postgres
        await prisma_1.prisma.roomBan.upsert({
            where: {
     ...
     */
    async unbanUser(moderatorId, roomId, targetUserId) {
        const isAllowed = await permission_service_1.permissionService.hasPermission(moderatorId, roomId, "UNBAN_USER");
        if (!isAllowed) {
            throw new Error("Unauthorized to unban user");
        }
        await prisma_1.prisma.roomBan.deleteMany({
            where: { roomId, userId: targetUserId },
        });
