...
const registerPlayerHandlers = (io, socket) => {
    ...
    socket.on(socket_constants_1.SOCKET_EVENTS.PLAYER_PLAY, async (payload, ack) => {
        await (0, socket_middleware_1.withErrorHandling)(async (_socket, data, ackFn) => {
            ...
            io.to(roomId).emit(socket_constants_1.SOCKET_EVENTS.PLAYER_STATE, state);
    ...
    socket.on(socket_constants_1.SOCKET_EVENTS.PLAYER_SYNC, async (payload, ack) => {
        await (0, socket_middleware_1.withErrorHandling)(async (_socket, data, ackFn) => {
